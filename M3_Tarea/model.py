...
class TrafficModel(mesa.Model):
  ...
  def __init__(self, n_agents_per_iter=2, max_agents=10, road_list=example, width=21, height=21):
    ...
    for road in road_list:
      ...
      while x != end[0]:
        ...
        if any(isinstance(elem, agent.Wall) for elem in cell):
          self.grid.remove_agent(cell[0])

        if any(isinstance(elem, agent.Road) for elem in cell):
          cell[0].dir.append(goes)
        else:
          r = agent.Road(0, self, x, y)
          r.dir.append(goes)
    ...
    # Create Lights
    self.traff = (9, 10)
    self.traffic_light = agent.Semaforo(0, self, self.traff, True)
    self.grid.place_agent(self.traffic_light, self.traff)

    self.traff2 = (10, 12)
    self.traffic_light2 = agent.Semaforo(1, self, self.traff2, False)
    ...
    for i in range(3):
      ...
      self.vehicle = agent.Carros(i, self, self.vehicle_pos)
    ...
    if self.iter == 5:
      ...
      for i in range(self.n_agents_per_iter):
        if self.n_agents == self.max_agents:
          ...
          if not any(isinstance(elem, agent.Carros) for elem in cell):
            self.vehicle = agent.Carros(i, self, pos[i])
